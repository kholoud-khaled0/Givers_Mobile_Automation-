...
public class OtpHelper {
    ...
    public static void enterOtpByViewGroupInstances(String otp) {

        try {
            AppiumDriver driver = DriverManager.getDriver();

            int startIndex = 18; // أول OTP box
            int otpLength = otp.length();

            for (int i = 0; i < otpLength; i++) {

                By boxLocator = AppiumBy.androidUIAutomator(
                        "new UiSelector().className(\"android.view.ViewGroup\").instance(" + (startIndex + i) + ")"
                );

                Waits.waitForElementToBeClickable(boxLocator, Duration.ofSeconds(5));
            ...
            System.out.println("✅ OTP entered successfully using ViewGroup instances: " + otp);

        } catch (Exception e) {
            throw new RuntimeException(
                    "Failed to enter OTP using ViewGroup instances", e
            );
        }
    }
    public static void enterOtpForJoinCommunity(String otp) {

        try {
            AppiumDriver driver = DriverManager.getDriver();

            // 1️⃣ اضغط على أي OTP box (instance 18 كفاية)
            By firstOtpBox = AppiumBy.androidUIAutomator(
                    "new UiSelector().className(\"android.view.ViewGroup\").instance(18)"
            );

            Waits.waitForElementToBeClickable(firstOtpBox, Duration.ofSeconds(10));
            driver.findElement(firstOtpBox).click();

            // 2️⃣ دور على أي EditText (المخفي الحقيقي)
            ...
            if (!inputs.isEmpty()) {
                ...
                // 3️⃣ fallback: اكتب في الـ active element
                driver.switchTo().activeElement().sendKeys(otp);
            }

            System.out.println("✅ OTP entered successfully (Join Community): " + otp);

        } catch (Exception e) {
            throw new RuntimeException(
                    "❌ Failed to enter OTP in Join Community flow", e
            );
        }
    }
